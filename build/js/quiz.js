// Quiz Section JavaScript - Integrated with existing site
class WasteQuiz {
  constructor() {
  ...
  updateCategoryContent() {
    ...
    if (countEl) countEl.textContent = `${categoryData.questions.length} Questions`;
    
    const difficulties = [...new Set(categoryData.questions.map(q => q.difficulty))];
    const difficultyText = difficulties.length > 1 ? 
      `${this.capitalize(difficulties[0])} to ${this.capitalize(difficulties[difficulties.length - 1])}` : 
      this.capitalize(difficulties[0]);
  ...
  startQuiz() {
    ...
    const selectionEl = document.getElementById('quiz-selection');
  ...
  loadQuestion() {
    ...
    if (questionNumEl) {
      questionNumEl.textContent = `Question ${this.currentQuestion + 1} of ${questions.length}`;
    }
    ...
    if (nextBtn) {
      ...
      nextBtn.textContent = this.currentQuestion === questions.length - 1 ? 'Finish Quiz' : 'Next Question';
    }
  }
  ...
  getCertificate() {
    ...
    const certificateHtml = `
      ...
      " onclick="this.remove()">
        ...
        ">
          <h2 style="color: var(--green-600); margin-bottom: 1rem;">🏆 Certificate of Achievement</h2>
          <p style="margin-bottom: 1rem;">This certifies that you have successfully completed the</p>
          <h3 style="color: var(--foreground); margin-bottom: 1rem;">${this.quizData[this.currentCategory].title}</h3>
          <p style="margin-bottom: 2rem;">with a passing score and are now a certified Green Champion!</p>
          <p style="font-size: 0.875rem; color: var(--muted-foreground);">Click anywhere to close</p>
        </div>
      </div>
    `;
